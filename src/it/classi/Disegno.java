...
public class Disegno {
	...
	public static void StampaStringaNotCap(String stringa) {
		int lunghezza = stringa.length();
		System.out.print("| ");
		System.out.print(stringa);
		for (int x = 0; x < lunghezzaMaxDisplay - lunghezza; x++) {
	...
	private static void ControlloQty(int x) {
		if (DistributoreAutomatico.distributore.get(x).size() == 0)
			System.out.print("--");
		else if (DistributoreAutomatico.distributore.get(x).size() < 10
				&& DistributoreAutomatico.distributore.get(x).size() > 0) {
			System.out.print("0");
		}
		if (DistributoreAutomatico.distributore.get(x).size() != 0) {
